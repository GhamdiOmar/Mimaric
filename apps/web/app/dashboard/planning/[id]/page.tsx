...
export default function PlanningWorkspaceDetailPage() {
  ...
  if (!workspace) {
    return (
      <div className="text-center py-20">
        <p className="text-neutral">{lang === "ar" ? "مساحة العمل غير موجودة" : "Workspace not found"}</p>
      </div>
    );
  }
  ...
  const tabs = [
    ...
    { key: "scenarios", label: { ar: "السيناريوهات", en: "Scenarios" }, icon: Stack },
    { key: "compliance", label: { ar: "الامتثال", en: "Compliance" }, icon: ShieldCheck },
    { key: "feasibility", label: { ar: "الجدوى", en: "Feasibility" }, icon: CurrencyCircleDollar },
    { key: "comparison", label: { ar: "المقارنة", en: "Compare" }, icon: ChartBar },
    { key: "comments", label: { ar: "التعليقات", en: "Comments" }, icon: ChatCircle },
  ];
              ...
              <button
                ...
                className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium border transition-colors whitespace-nowrap ${
                  activeScenarioId === s.id
                    ...
                    : "border-border text-neutral hover:border-secondary/30"
                }`}
                style={{ display: "inline-flex" }}
              >
                {s.isBaseline && <Star size={12} weight="fill" className="text-amber-500" />}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
          {[
            ...
            <div key={i} className="bg-card border border-border rounded-lg p-3">
              <p className="text-[10px] text-neutral font-medium">{card.label[lang]}</p>
          ...
          return (
            ...
            >
              ...
              {tab.key === "comments" && comments.length > 0 && (
                <span className="bg-muted text-neutral rounded-full px-1.5 text-[9px]">{comments.length}</span>
              )}
            </button>
          );
        })}
          ...
          <div className="space-y-4">
            ...
            <div className="bg-card border border-border rounded-xl p-4">
              <h3 className="text-sm font-bold text-primary mb-3 flex items-center gap-2">
                <Stack size={16} />
                {lang === "ar" ? "الطبقات" : "Stack"}
              </h3>
              <div className="space-y-2">
                {workspace.spatialStack?.map((layer: any) => (
                  <div key={layer.id} className="flex items-center justify-between py-1.5 px-2 rounded hover:bg-muted/50">
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {layer.visible && <CheckCircle size={14} className="text-white" weight="fill" />}
                      </button>
                      <div
                      ...
                      <span className="text-[10px] text-neutral">({layer.featureCount})</span>
                    </div>
                    <button
                      onClick={async () => { await deleteSpatialLayer(layer.id); loadWorkspace(); }}
                      className="text-neutral hover:text-destructive p-1"
                    >
                      <Trash size={12} />
                    </button>
                  </div>
                ))}
                {(!workspace.spatialStack || workspace.spatialStack.length === 0) && (
                  <p className="text-xs text-neutral py-2">{lang === "ar" ? "لا توجد طبقات. استورد ملفات مكانية." : "No layers. Import spatial files."}</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
              ...
              return (
                <div key={s.id} className={`bg-card border rounded-xl p-4 ${s.id === activeScenarioId ? "border-secondary" : "border-border"}`}>
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-2">
                        {s.isBaseline && <Star size={14} weight="fill" className="text-amber-500" />}
                        ...
                        <span className="text-[10px] text-neutral">v{s.version}</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-1">
                      <button onClick={() => setActiveScenarioId(s.id)} className="p-1.5 text-neutral hover:text-primary" title="View">
                        <Eye size={14} />
                      </button>
                      <button onClick={() => handleDuplicate(s.id)} className="p-1.5 text-neutral hover:text-primary" title="Duplicate">
                        <Copy size={14} />
                      </button>
                      {s.status === "DRAFT" && (
                        <button
                          onClick={async () => { await updateScenarioStatus(s.id, "UNDER_REVIEW"); loadWorkspace(); }}
                          className="p-1.5 text-neutral hover:text-yellow-600" title="Submit for Review"
                        >
                          <Clock size={14} />
                        </button>
                      )}
                      {s.status === "UNDER_REVIEW" && (
                        <>
                          <button
                            ...
                            className="p-1.5 text-neutral hover:text-green-600" title="Approve as Baseline"
                          >
                            <CheckCircle size={14} />
                          </button>
                          <button
                            onClick={async () => { await updateScenarioStatus(s.id, "REJECTED"); loadWorkspace(); }}
                            className="p-1.5 text-neutral hover:text-red-600" title="Reject"
                          >
                            <X size={14} />
                          </button>
                        </>
                      ...
                      <button
                        ...
                        className="p-1.5 text-neutral hover:text-destructive" title="Delete"
                      >
                        <Trash size={14} />
                      </button>
                    </div>
                  ...
                  {subPlan && (
                    <div className="grid grid-cols-4 gap-3 mt-3 text-center">
                      <div>
                        ...
                        <p className="text-[10px] text-neutral">{lang === "ar" ? "قطع" : "Plots"}</p>
                      </div>
                      <div>
                        <p className="text-lg font-bold text-primary">{subPlan.blocks?.length ?? 0}</p>
                        <p className="text-[10px] text-neutral">{lang === "ar" ? "بلوكات" : "Blocks"}</p>
                      </div>
                      <div>
                        <p className="text-lg font-bold text-primary">{subPlan.roads?.length ?? 0}</p>
                        <p className="text-[10px] text-neutral">{lang === "ar" ? "طرق" : "Roads"}</p>
                      </div>
                      <div>
                        <p className="text-lg font-bold text-primary">{s._count?.complianceResults ?? 0}</p>
                        <p className="text-[10px] text-neutral">{lang === "ar" ? "فحوصات" : "Checks"}</p>
                      </div>
                    </div>
                  )}
                </div>
            ...
            {(!workspace.scenarios || workspace.scenarios.length === 0) && (
              <div className="text-center py-12 bg-card border border-border rounded-xl">
                <Stack size={40} className="mx-auto text-neutral/40 mb-2" />
                <p className="text-neutral">{lang === "ar" ? "لا توجد سيناريوهات" : "No scenarios yet"}</p>
                <button
                  onClick={() => setShowNewScenario(true)}
        ...
        {activeTab === "compliance" && (
          <div className="space-y-4">
            ...
            {complianceSummary && (
              <div className="grid grid-cols-4 gap-3">
                <div className="bg-card border border-border rounded-lg p-3 text-center">
                  ...
                  <p className="text-[10px] text-neutral">{lang === "ar" ? "نسبة الامتثال" : "Compliance Score"}</p>
                </div>
                ...
                <div className="bg-card border border-border rounded-lg p-3 text-center">
                  <p className="text-2xl font-bold text-neutral">{complianceSummary.total}</p>
                  <p className="text-[10px] text-neutral">{lang === "ar" ? "إجمالي" : "Total"}</p>
                </div>
              </div>
            )}
              ...
              <div className="bg-card border border-border rounded-xl overflow-hidden">
                <table className="w-full text-xs">
                  <thead className="bg-muted/50">
                    <tr>
                      <th className="text-start px-3 py-2 text-neutral font-medium">{lang === "ar" ? "الحالة" : "Status"}</th>
                      <th className="text-start px-3 py-2 text-neutral font-medium">{lang === "ar" ? "القاعدة" : "Rule"}</th>
                      <th className="text-start px-3 py-2 text-neutral font-medium">{lang === "ar" ? "العنصر" : "Feature"}</th>
                      <th className="text-start px-3 py-2 text-neutral font-medium">{lang === "ar" ? "الفعلي" : "Actual"}</th>
                      <th className="text-start px-3 py-2 text-neutral font-medium">{lang === "ar" ? "المطلوب" : "Expected"}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {complianceResults.map((r: any) => (
                      <tr key={r.id} className="border-t border-border">
                        ...
                        <td className="px-3 py-2 text-neutral">{r.featureLabel}</td>
                        <td className="px-3 py-2 text-primary font-medium">{r.actualValue}</td>
                        <td className="px-3 py-2 text-neutral">{r.expectedValue}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            {complianceResults.length === 0 && !complianceSummary && (
              <div className="text-center py-12 bg-card border border-border rounded-xl">
                <ShieldCheck size={40} className="mx-auto text-neutral/40 mb-2" />
                <p className="text-neutral">{lang === "ar" ? "لم يتم تشغيل الفحص بعد" : "No compliance check run yet"}</p>
              </div>
            )}
          </div>
        )}
            ...
            {activeScenario?.feasibilitySet?.calculatedMetrics ? (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  ...
                  <div key={i} className="bg-card border border-border rounded-lg p-3">
                    <p className="text-[10px] text-neutral">{item.label[lang]}</p>
              ...
              <div className="text-center py-12 bg-card border border-border rounded-xl">
                <CurrencyCircleDollar size={40} className="mx-auto text-neutral/40 mb-2" />
                <p className="text-neutral">{lang === "ar" ? "لم تتم إضافة افتراضات الجدوى" : "No feasibility assumptions set"}</p>
              </div>
            )}
          </div>
        )}
            ...
            {workspace.scenarios?.length >= 2 ? (
              <div className="overflow-x-auto">
                <table className="w-full text-xs bg-card border border-border rounded-xl overflow-hidden">
                  <thead className="bg-muted/50">
                    <tr>
                      <th className="text-start px-3 py-2.5 text-neutral font-medium min-w-[140px]">{lang === "ar" ? "المقياس" : "Metric"}</th>
                      {workspace.scenarios.map((s: any) => (
                        <th key={s.id} className="text-center px-3 py-2.5 text-primary font-medium min-w-[120px]">
                          {s.isBaseline && <Star size={10} weight="fill" className="inline text-amber-500 me-1" />}
                          {s.name}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {[
                      ...
                      <tr key={row.key} className="border-t border-border">
                        <td className="px-3 py-2 text-neutral font-medium">{row.label[lang]}</td>
                        {workspace.scenarios.map((s: any) => {
                          const m = s.metrics || {};
                          let val: any = "—";
                          if (row.key === "status") {
              ...
              <div className="text-center py-12 bg-card border border-border rounded-xl">
                <ChartBar size={40} className="mx-auto text-neutral/40 mb-2" />
                <p className="text-neutral">{lang === "ar" ? "أنشئ سيناريوهين على الأقل للمقارنة" : "Create at least 2 scenarios to compare"}</p>
              </div>
            )}
          </div>
        )}
              ...
              {comments.map((c: any) => (
                <div key={c.id} className="bg-card border border-border rounded-lg p-3">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[10px] text-neutral">{c.authorRole}</span>
                    <span className="text-[10px] text-neutral/60">{new Date(c.createdAt).toLocaleString(lang === "ar" ? "ar-SA" : "en-US")}</span>
                  </div>
                  <p className="text-sm text-primary">{c.content}</p>
                </div>
              ))}
              {comments.length === 0 && (
                <p className="text-center text-neutral text-sm py-8">{lang === "ar" ? "لا توجد تعليقات" : "No comments yet"}</p>
              )}
            </div>
          </div>
        )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-overlay/60 backdrop-blur-sm">
          <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6">
            ...
            <div className="border-2 border-dashed border-border rounded-xl p-8 text-center">
              <Upload size={32} className="mx-auto text-neutral/40 mb-2" />
              <input
                type="file"
            ...
            <div className="flex justify-end mt-4">
              <button onClick={() => setShowImport(false)} className="px-4 py-2 text-sm text-neutral hover:text-primary" style={{ display: "inline-flex" }}>
                {lang === "ar" ? "إغلاق" : "Close"}
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-overlay/60 backdrop-blur-sm">
          <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-lg mx-4 p-6">
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button onClick={() => setShowFeasibility(false)} className="px-4 py-2 text-sm text-neutral hover:text-primary" style={{ display: "inline-flex" }}>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-overlay/60 backdrop-blur-sm">
      <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6">
        ...
        <div className="flex justify-end gap-3 mt-5">
          <button onClick={onClose} className="px-4 py-2 text-sm text-neutral" style={{ display: "inline-flex" }}>{lang === "ar" ? "إلغاء" : "Cancel"}</button>
...
function PlanningMapCanvas({ workspace, scenario, onRefresh, lang }: { workspace: any; scenario: any; onRefresh: () => void; lang: string }) {
  ...
  React.useEffect(() => {
    ...
    import("leaflet").then((L) => {
      ...
      if (workspace.spatialStack) {
        for (const layer of workspace.spatialStack) {
  ...
  }, [mapReady, scenario?.id, workspace?.spatialStack]);
